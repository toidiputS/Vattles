...
import { LogoIcon, TrophyIcon, SparklesIcon, UserIcon, PinIcon, ShieldCheckIcon, BrainCircuitIcon, FirstWinIcon, WinStreakIcon, CyberpunkMasterIcon, PromptVirtuosoIcon, HandThumbUpIcon, SwordsIcon, MedalIcon, FounderBadge, WarStarterBadge } from './icons';
...
const ProfileView: React.FC<ProfileViewProps> = ({ userProfile, onBack, onEdit, onPinItem, onUnpinItem, onUpdateVibeAnalysis, onUpdateProfile }) => {
    ...
    return (
        <div className={`w-full max-w-5xl mx-auto animate-fade-in ${currentTheme.classes}`}>
            ...
            <main className="space-y-12">
                <section className={`${currentTheme.sectionClasses} p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-6`}>
                    <div className="relative">
                        <img src={avatarUrl} alt={name} className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 shadow-lg ${userProfile.isWarStarter ? 'border-orange-500 shadow-orange-900/60' : status === 'pro' ? `border-cyan-400 shadow-cyan-900/60` : currentTheme.headerClasses}`} />
                        {userProfile.isWarStarter && (
                            <div className="absolute -top-3 -right-3 z-20">
                                <ShimmerDust color="#FB923C" count={15}>
                                    <WarStarterBadge className="h-12 w-12 drop-shadow-[0_0_15px_#FB923C]" />
                                </ShimmerDust>
                            </div>
                        )}
                        {userProfile.isFounder && !userProfile.isWarStarter && <FounderBadge className="absolute -top-2 -right-2 h-8 w-8 drop-shadow-[0_0_8px_#FCD34D] animate-pulse" />}
                    </div>
                    <div className="text-center sm:text-left grow">
                        <div className="flex items-center gap-4 justify-center sm:justify-start flex-wrap">
                            ...
                            <div className="flex items-center gap-2">
                                <MedalIcon tier={userProfile.rankTier} className="h-8 w-8 drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]" />
                                <span className={`text-sm font-bold uppercase tracking-widest ${userProfile.rankTier === 'medal-of-honor' ? 'text-red-400 animate-pulse' : 'text-gray-300'}`}>
                                    {userProfile.rankTier.replace('-', ' ')}
                                </span>
                            </div>
                            {status === 'pro' && (
                            ...
                            {userProfile.isWarStarter && (
                                <div className="flex items-center gap-2 text-orange-400 bg-orange-900/30 border border-orange-700/50 px-3 py-1 rounded-full text-sm font-bold animate-pulse shadow-[0_0_10px_rgba(251,146,60,0.3)]">
                                    WAR STARTER
                                </div>
                            )}
                            {userProfile.isFounder && !userProfile.isWarStarter && (
                                <div className="flex items-center gap-2 text-yellow-500 bg-yellow-900/30 border border-yellow-700/50 px-3 py-1 rounded-full text-sm font-bold">
                                    FOUNDER EDITION
                                </div>
                            )}
                        </div>
                        {userProfile.isWarStarter && (
                            <p className="text-orange-300/80 text-xs font-bold uppercase tracking-[0.2em] mt-2 mb-1 italic">
                                "You may have won a Vattle but my First Prompt Started This War"
                            </p>
                        )}
                        <div className="mt-4 flex flex-wrap gap-6 justify-center sm:justify-start">
                            <p className={`${currentTheme.accentText} font-semibold flex items-center gap-2`}>
                                <TrophyIcon className="h-5 w-5 opacity-60" />
                                MMR: <span className="text-white">{stats.mmr}</span>
                            </p>
                            <p className={`${currentTheme.accentText} font-semibold`}>Member Since: {new Date(joinDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        </div>
                    </div>
                </section>
                        ...
                        <div className="flex flex-wrap gap-4">
                            {Object.entries(themes).map(([key, theme]) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg font-semibold border-2 transition-colors ${userProfile.profileTheme === key
